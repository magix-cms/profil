<form id="links-form" method="post" action="{$smarty.server.REQUEST_URI}">
    ...
    <div class="row">
        {foreach $socials as $link}
            {$label = 'pn_profil_'|cat:$link}
            {$placeholder = 'ph_profil_'|cat:$link}
            <div class="form-group col-xs-12 col-sm-6">
                ...
                <div class="input-group links-icon">
                    ...
                    <input id="{$link}_pr" type="text" name="{$link}_pr" value="{$data[{$link}]}" placeholder="{#$placeholder#|ucfirst}" class="form-control" aria-describedby="sizing-addon1"/>
                </div>
            </div>
        {/foreach}
    </div>
