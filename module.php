...
class plugins_profil_module extends database_plugins_profil {
	...
	public function register($module){
		if(parent::c_show_table('mc_plugins_profil_module')) {
			if(parent::g_mod($module) == null) {
				$module_class = 'plugins_'.$module.'_profil';
	...
	private function get_plugins($root){
		...
		{
			if (!in_array($value,array(".","..","profil")))
			{
	...
	private function extend_module($plugins = null){
		...
		foreach ($plugins as $plugin) {
			if(!parent::g_mod($plugin)) {
				if(file_exists($this->pRoot.$plugin.DIRECTORY_SEPARATOR.'profil.php')){
					if(class_exists('plugins_'.$plugin.'_profil')){
						$this->register($plugin);
					}
				}
			} else {
				if(!file_exists($this->pRoot.$plugin.DIRECTORY_SEPARATOR.'profil.php') || !class_exists('plugins_'.$plugin.'_profil')){
	...
	public function load_module($parse_module){
		...
		foreach ( $modules as $mod ) {
			$modClass = 'plugins_'.$mod['module_name'].'_profil';
